...
describe('AccountMongoRepository', () => {
  ...
  describe('updateAccessToken()', () => {
    test('Should update the account accessToken on success', async () => {
      const sut = makeSut();
      const { _id } = await accountCollection.create(mockAddAccountParams());
      const fakeAccount: any = await accountCollection.findOne({ _id }).exec();
      expect(fakeAccount.accessToken).toBeFalsy();
      const accessToken = faker.datatype.uuid();
      await sut.updateAccessToken(fakeAccount._id, accessToken);
      const account: any = await accountCollection.findOne({ _id: fakeAccount._id });
      expect(account).toBeTruthy();
